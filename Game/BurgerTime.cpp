...
void BurgerTime::Initialize() {
  dae::ServiceProvider::RegisterSoundSystem(
      std::make_unique<dae::SDL_SoundSystem>(
...
void BurgerTime::Destroy() { dae::ServiceProvider::GetSoundSystem().Destroy(); }
...
void BurgerTime::SetupKeybinds() {
  ...
  for (int i = 0; i < 4; ++i) {
    inputManager.GetControllerInput(i)
        ...
        .BindInputAction(InputAction::Attack, InputKeybinds::BUTTON_WEST);
  }

  auto keyboardInput{inputManager.GetKeyboardInput()};
  keyboardInput->AddBinding(std::make_unique<dae::ToggleSoundCommand>(),
                            InputKeybinds::F2, InputState::JustPressed);
}
