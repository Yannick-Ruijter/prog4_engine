...
Scene *dae::GameSceneLoader::LoadScene(LevelInfo levelInfo) {
  ...
  dae::Subject *player2PickUpSubject2{nullptr};
  {
    ...
    auto player2DamageBinding = player2Input->AddBinding(
        std::make_unique<dae::DamagePlayer>(*player2, *player1),
        InputKeybinds::C, InputState::JustPressed);
    auto player2PickUpBinding1 = player2Input->AddBinding(
        std::make_unique<dae::PickUpItemCommand>(*player2), InputKeybinds::Z,
        InputState::JustPressed);
    auto player2PickUpBinding2 = player2Input->AddBinding(
        std::make_unique<dae::PickUpItemCommand>(*player2), InputKeybinds::X,
        InputState::JustPressed);
  ...
  // lives display
  {
    ...
    player1->GetComponent<dae::Score>()->GetSubject()->AddObserver(
        go->GetComponent<dae::ScoreDisplay>());
    scene->Add(std::move(go));
    ...
    player2->GetComponent<dae::Score>()->GetSubject()->AddObserver(
        go->GetComponent<dae::ScoreDisplay>());
    scene->Add(std::move(go));
  }

  LoadSpriteMap(scene, tileSize,
                std::vector<GameObject *>{player1.get(), player2.get()},
                levelGrid);
