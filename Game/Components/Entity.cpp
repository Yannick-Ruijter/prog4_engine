...
Entity::Entity(GameObject &owner, std::unique_ptr<InputProvider> input, LevelGrid *level)
    : Component(owner),
      m_SpriteAnimation{owner.GetComponent<SpriteAnimation>()},
      m_CurrentState{std::make_unique<PlayerStateIdle>(*this)},
      m_Input{std::move(input)},
      m_Level{level} {
    assert(m_SpriteAnimation != nullptr && "Entity needs a SpriteAnimation (added before this component)");
}
...
GameObject *dae::Entity::GetPlayer() {
    return GetOwner();
}
...
InputProvider *dae::Entity::GetInput() const {
    return m_Input.get();
}

LevelGrid *dae::Entity::GetLevel() const {
    return m_Level;
}

float dae::Entity::GetMoveSpeed() const {
    return m_MoveSpeed;
}
