...
public:
  Impl(GameObject &owner, std::string const &animationDataPath,
...
private:
  ...
  Texture2DDisplay *m_CurrentPlayerTexture{nullptr};
  bool m_HasState{false};

  void InitStateInfo();
};

SpriteAnimation::SpriteAnimation(GameObject &owner,
                                 std::string const &animationDataPath,
                                 std::string const &spriteSheetPath)
    : Component(owner), m_pImpl{std::make_unique<Impl>(owner, animationDataPath,
                                                       spriteSheetPath)} {}

dae::SpriteAnimation::~SpriteAnimation() = default;

void dae::SpriteAnimation::Update() { m_pImpl->Update(); }

void dae::SpriteAnimation::SetAnimationState(std::string const &animState) {
  m_pImpl->SetAnimationState(animState);
}

dae::SpriteAnimation::Impl::Impl(GameObject &owner,
                                 std::string const &animationDataPath,
                                 std::string const &spriteSheetPath)
    : m_Owner{&owner} {
...
void dae::SpriteAnimation::Impl::InitAnimationFrame() {
  SDL_Surface *animationFrame = SDL_CreateSurface(
      m_CurrentState->dimensions.x * 2, m_CurrentState->dimensions.y * 2,
      m_SpriteSheetSurface->format);

  SDL_Rect frameRect{
      m_CurrentState->startPos.x +
          (m_CurrentAnimationFrame * m_CurrentState->dimensions.x),
      m_CurrentState->startPos.y, m_CurrentState->dimensions.x,
      m_CurrentState->dimensions.y};

  SDL_Rect destRect{0, 0, m_CurrentState->dimensions.x * 2,
                    m_CurrentState->dimensions.y * 2};

  SDL_BlitSurfaceScaled(m_SpriteSheetSurface, &frameRect, animationFrame,
                        &destRect, SDL_SCALEMODE_NEAREST);

  SDL_Texture *texture = SDL_CreateTextureFromSurface(
      Renderer::GetInstance().GetSDLRenderer(), animationFrame);
