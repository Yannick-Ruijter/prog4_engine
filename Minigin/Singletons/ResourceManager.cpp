...
std::shared_ptr<dae::Texture2DDisplay>
dae::ResourceManager::LoadTexture(GameObject *parent, const std::string &file) {
    const auto fullPath = m_dataPath / file;
    const auto filename = fs::path(fullPath).filename().string();
    if (m_loadedTextures.find(filename) == m_loadedTextures.end())
        m_loadedTextures.insert(std::pair(filename, std::make_shared<Texture2DDisplay>(*parent, fullPath.string())));
